...
@Entity(tableName = "interested_events")
data class InterestedEventEntity(
    @PrimaryKey val id: String,
    val title: String,
    val sportCategory: String,
    val logoUrl: String,
    val feedsJson: String,
    val startTime: Long, // Alert target timestamp in milliseconds
    ...
    companion object {
        ...
        fun fromGroupedEvent(event: GroupedEvent, startTime: Long): InterestedEventEntity {
            val json = try {
                adapter.toJson(event.feeds)
            } catch (e: Exception) {
                "[]"
            }
            return InterestedEventEntity(
                id = event.id,
                title = event.title,
